...
Не все геоточки описывают движение цели. События «работа ПВО», «взрыв», «падение обломков» фиксируют дискретное состояние в пространстве, но не должны корректировать вектор скорости летящего объекта. Если передать их в `kalman.correct()`, фильтр обнулит скорость и сломает прогноз.
...
| `attach_only` | **не вызывается** | Стационарные: ПВО, взрыв, impact |
...
- Integration: трек с ПВО-точкой посередине сохраняет velocity через узел.
