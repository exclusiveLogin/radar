...
1. **Хранение raw-данных:** `mat_ingest_raw.rawPayload` (JSONB) — полный оригинал от источника; O2O `mat_ingest_raw_tg` — только индексная выжимка для dedup.
...
3. **Live-курсор:** `state_ingest_cursor` хранит прогресс по `(channelId, providerKey)`. При холодном запуске курсора нет → adapter слушает **только новые** сообщения (не качает историю с начала).
