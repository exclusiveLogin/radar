...
- Для оффлайн проверки этого же боевого пайплайна используйте `npm run worker:parse:report`:
  - вход по умолчанию `tests/`;
  - выход по умолчанию `reports/`;
  - резолв идет от `artifacts` (region.csv + Cities GeoJSON), затем optional enrich по флагу.
