...
async function main(): Promise<void> {
  ...
  try {
    ...
    await client.query(`DELETE FROM place_status_active WHERE place_id = ANY($1::uuid[])`, [losers]);
    await client.query(`DELETE FROM place_status_history WHERE place_id = ANY($1::uuid[])`, [losers]);
