...
async function main(): Promise<void> {
  ...
  const byStatus = await ds.query(
    `SELECT provider, status, COUNT(*)::int AS n
     FROM place_enrichment_jobs GROUP BY provider, status ORDER BY provider, status`,
  );
  console.log("jobs by provider/status:", byStatus);

  const stuckProcessing = await ds.query(
    `SELECT COUNT(*)::int AS n FROM place_enrichment_jobs
  ...
  console.log("active phase_runs:", runs);
  ...
  const doneA = (await ds.query(
    `SELECT COUNT(*)::int AS n FROM place_enrichment_jobs WHERE provider='dadata' AND status='done'`,
  )) as Array<{ n: number }>;
