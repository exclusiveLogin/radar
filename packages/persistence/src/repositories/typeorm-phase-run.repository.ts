...
/** Запуски фаз (log_phase_run): прогресс, лог, control. */
export class TypeOrmPhaseRunRepository implements IPhaseRunRepository {
  ...
  async create(input: {
    ...
    const rows = readTypeOrmQueryRows<RunRow>(
      await this.dataSource.query(
        `INSERT INTO log_phase_run (phase_id, trigger, status, started_at)
  ...
  async findById(id: string): Promise<PhaseRun | null> {
    const rows = (await this.dataSource.query(`SELECT * FROM log_phase_run WHERE id = $1`, [
      id,
    ])) as RunRow[];
  ...
  async updateStats(id: string, stats: PhaseRunStats): Promise<void> {
    await this.dataSource.query(
      `UPDATE log_phase_run SET stats = $2::jsonb, updated_at = now() WHERE id = $1`,
  ...
  async requestControl(id: string, control: PhaseRunControl): Promise<void> {
    await this.dataSource.query(
      `UPDATE log_phase_run SET control = $2, updated_at = now() WHERE id = $1`,
      [id, control],
    );
  }
  ...
  async clearControl(id: string): Promise<void> {
    await this.dataSource.query(
      `UPDATE log_phase_run SET control = NULL, updated_at = now() WHERE id = $1`,
      [id],
    );
  }
  ...
  async getControl(id: string): Promise<PhaseRunControl | null> {
    const rows = (await this.dataSource.query(
      `SELECT control FROM log_phase_run WHERE id = $1`,
      [id],
